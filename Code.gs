...
function renderSchedule_(sched, schedule) {
  ...
  ensureSheetSize_(sched, output.length + 1, timeline.length + 8);

  sched.getRange(1, 1, 1, 8).setValues([['Activity No.', 'Activity', 'Predecessor', 'Duration', 'Early Start', 'Early Finish', 'Late Start', 'Late Finish']]);
  sched.getRange(2, 1, output.length, 8).setValues(output);

  sched.getRange(1, 9, 1, timeline.length).setValues([timeline]);
  ...
  sched.getRange(2, 9, backgrounds.length, timeline.length).setBackgrounds(backgrounds);
}
